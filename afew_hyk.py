...
parser.add_argument('-p', '--patch_size', type=int, default=2)
args = parser.parse_args()


def main():
    start_date = util.time_now()
    logger = util.Logger('./log/', 'afew_hyk', start_date, args.aggregate_mode)
    logger.print(
        'The aggregate mode is {:}, patch size is {:} ,learning rate: {:}'.format(args.aggregate_mode, args.patch_size,
                                                                                  args.learning_rate))
    # 加载cheafewpoint
    save_path = './model/afew_hyk_' + args.aggregate_mode + '_' + args.save_suffix + '.pth.tar'
    ...
    if args.resume:
        ...
        logger.print('train from afew_hyk_' + args.aggregate_mode + '_' + args.save_suffix)
    else:
    ...
    # 加载数据
    video_root = './data/face/afew_face'
    video_list = './data/txt/afew_train.txt'
    points_name = '16pts.list'
    batch_train = args.batch_train
    batch_eval = 64
    num_workers = args.num_workers

    train_loader, val_loader = load.afew_faces_hyk(video_root, video_list, points_name, num_workers, batch_train,
...
def train(train_loader, model, optimizer, epoch, logger, fold):
    ...
    for i, (input_first, input_second, input_third, target, ponits_three, video_index) in enumerate(train_loader):
        ...
        input_var = torch.staafew([input_first, input_second, input_third], dim=4).to(DEVICE_CUDA)
